...
def realizar_analisis_especifico(tabla_datos: pd.DataFrame, base_datos: gffutils.FeatureDB, ruta_salida: Path):
    """
    Genera visualizaciones biológicas y estadísticas específicas para sitios de splicing.
    """
    ...
    etiquetas_bonitas = ["Sitios Reales (Donantes)", "Sitios Señuelo (Falsos GT)"]
    plt.pie(conteo_clases, labels=etiquetas_bonitas, autopct='%1.1f%%', colors=['#66b3ff','#99ff99'])
    plt.title("Balance de Clases en el Dataset Consolidado")
    ...
    plt.title("Distribución Top 10: Densidad de Sitios por Cromosoma")
    plt.xlabel(ETIQUETA_FRECUENCIA)
    ...
    # 3. Distribución de Contenido GC (Violin Plot)
    ...
    plt.figure()
    # Usar etiquetas descriptivas en lugar de 0/1
    ...
    plt.title("Comparativa de Contenido GC por Naturaleza del Sitio")
    plt.ylabel("Fracción de Guanina-Citosina")
    plt.savefig(ruta_salida / "spec_04_gc_content_violin.png")
    plt.close()

    # 4. Longitud de exones (Referencia Genómica)
    ...
    plt.xlim(0, 1000)
    plt.title("Perfil de Longitud de Exones (Anotación GENCODE)")
